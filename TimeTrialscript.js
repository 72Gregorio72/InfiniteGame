let timer = 180;  // Tempo iniziale (3 minuti)
let lives = 3;   // Numero di vite iniziale
let level = 1;   // Livello corrente

function giocoTempoLimitato() {
  setInterval(function() {
    if (timer <= 0 || lives === 0) {
      fineGioco();
    } else {
      console.log(`Livello ${level}`);
      console.log(`Tempo rimanente: ${Math.floor(timer / 60)}:${timer % 60 < 10 ? '0' : ''}${timer % 60}`);
      console.log(`Vite rimanenti: ${lives}`);
    ...
    while (numBlocks < 20) {
        ...
        var randomWidth = Math.random() * 150 + minDistance; // Larghezza casuale
        var block = { x: randomX, y: randomY, width: randomWidth, height: 20 };
    ...
    window.location.href = url;
      ...
      let completato = true;  // Indica se il livello è stato completato
      
      if (completato) {
        timer += 15;  // Aumenta il timer di 15 secondi
        level++;
      } else {
...
function fineGioco() {
  ...
  // Altro codice per la fine del gioco
}
